...
export default function KonvaLabelEditor() {
  ...
  const populateProductData = (elements: any[], product: any) => {
    // First, replace any existing placeholders
    ...
    // Check if we need to add auto-generated product fields
    const hasProductName = updatedElements.some(el =>
      (el.type === 'text' || el.type === 'i-text') &&
      (el.text?.includes(product.productName) || el.text?.includes('{{productName}}'))
    );

    // If no product data fields exist, auto-generate them
    if (!hasProductName && product.productName) {
      logger.debug('Auto-generating product data fields');
      ...
      const autoGeneratedFields = [
        ...
        {
          type: 'text',
          text: `Barcode: ${product.gs1BarcodeNumber}`,
          left: 50,
          top: 130,
          fontSize: 12,
          fontFamily: 'Courier',
      ...
      return [...updatedElements, ...autoGeneratedFields];
    }
